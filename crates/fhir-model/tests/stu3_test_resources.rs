#![cfg(all(feature = "stu3", feature = "builders"))]
...
fn serialization_deserialization() {
	for entry in fs::read_dir(format!("{}/tests/stu3-examples-json/", env!("CARGO_MANIFEST_DIR")))
		.expect("read dir")
	{
...
#[test]
fn coding_concepts() {
	let code = ExtraActivityType::_Custom("Test".to_owned());
...
fn reference_parsing() {
	...
	let reference = Reference::builder().reference("Encounter/1".to_owned()).build().unwrap();
	let parsed = reference.parse().expect("parsing reference");
	assert_eq!(
		parsed,
	...
	let reference =
		Reference::builder().reference("http://not-fhir.test/1".to_owned()).build().unwrap();
	let parsed = reference.parse().expect("parsing reference");
	assert_eq!(
		parsed,
		ParsedReference::Absolute {
